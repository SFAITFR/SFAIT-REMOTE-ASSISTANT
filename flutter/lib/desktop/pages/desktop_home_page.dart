...
class _DesktopHomePageState extends State<DesktopHomePage>
  ...
  bool _startupUpdateGateReleased = false;
  ...
  Future<void> _releaseStartupUpdateGate() async {
    if (_startupUpdateGateReleased || !mounted) {
      return;
    }
    _startupUpdateGateReleased = true;
    _block.value = false;
  }

  Future<void> _quitForRequiredUpdate() async {
    try {
      await rustDeskWinManager.closeAllSubWindows();
    } catch (_) {}
    ...
    if (isMacOS) {
      periodic_immediate(
          const Duration(milliseconds: 30), RdPlatformChannel.instance.terminate);
    } else if (isWindows) {
      exit(0);
    }
  }

  Future<void> _showStartupUpdatePrompt(String updateUrl) async {
    if (!mounted || _startupUpdatePromptShown) {
      return;
    }
    ...
    gFFI.dialogManager.show((setState, close, context) {
      return CustomAlertDialog(
        title: Text(translate('Status')),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
              '${translate("new-version-of-{${appName}}-tip")} (${detectedVersion.isEmpty ? "?" : detectedVersion})',
              style: const TextStyle(fontSize: 15),
            ),
            const SizedBox(height: 12),
            Text(
              'La mise a jour est verifiee au lancement de l’application.',
              style: TextStyle(
                fontSize: 13,
                color: Theme.of(context)
                    .textTheme
        ...
        actions: [
          dialogButton(
            'Quitter SFAIT Remote Assistant',
            onPressed: () async {
              close();
              await _quitForRequiredUpdate();
            },
            isOutline: true,
          ),
          dialogButton(
    ...
    }, tag: 'startup-force-update');
  }

  Future<void> _runStartupUpdateGate() async {
    ...
    while (mounted && DateTime.now().isBefore(deadline)) {
      ...
      final effectiveUpdateUrl = _getEffectiveUpdateUrl();
      if (effectiveUpdateUrl.isNotEmpty) {
        await _showStartupUpdatePrompt(effectiveUpdateUrl);
        return;
      }
      await Future.delayed(const Duration(milliseconds: 350));
    }

    await _releaseStartupUpdateGate();
  }
