...
pub fn global_init() -> bool {
    *config::APP_NAME.write().unwrap() = crate::brand::APP_NAME.to_owned();
    #[cfg(target_os = "macos")]
    {
        *config::ORG.write().unwrap() = crate::brand::BUNDLE_ORG.to_owned();
    }
    *config::PROD_RENDEZVOUS_SERVER.write().unwrap() = crate::brand::SERVER_HOST.to_owned();
    {
        let mut hard_settings = config::HARD_SETTINGS.write().unwrap();
        hard_settings.insert(
            keys::OPTION_CUSTOM_RENDEZVOUS_SERVER.to_owned(),
            crate::brand::SERVER_HOST.to_owned(),
        );
        hard_settings.insert(
            keys::OPTION_RELAY_SERVER.to_owned(),
            crate::brand::RELAY_HOST.to_owned(),
        );
        hard_settings.insert(
            keys::OPTION_API_SERVER.to_owned(),
            crate::brand::API_SERVER.to_owned(),
        );
        hard_settings.insert("key".to_owned(), crate::brand::SERVER_KEY.to_owned());
        hard_settings.insert(
            keys::OPTION_ALLOW_AUTO_UPDATE.to_owned(),
            "Y".to_owned(),
        );
    }
    // Make the branded server settings visible and effective from the first launch,
    // including in desktop UI flows that read persisted options directly.
    Config::set_option(
        keys::OPTION_CUSTOM_RENDEZVOUS_SERVER.to_owned(),
        crate::brand::SERVER_HOST.to_owned(),
    );
    ...
    Config::set_option("key".to_owned(), crate::brand::SERVER_KEY.to_owned());
    ...
    #[cfg(target_os = "macos")]
    {
        // Texture rendering is flaky on this custom macOS build and can lead to
        // black remote screens while input still works, so keep it disabled.
        LocalConfig::set_option(keys::OPTION_TEXTURE_RENDER.to_owned(), "N".to_owned());
    }
    #[cfg(target_os = "linux")]
    {
...
pub fn get_uri_prefix() -> String {
    format!("{}://", crate::brand::URI_SCHEME)
}

#[cfg(target_os = "macos")]
pub fn get_full_name() -> String {
    crate::brand::BUNDLE_IDENTIFIER.to_owned()
}
