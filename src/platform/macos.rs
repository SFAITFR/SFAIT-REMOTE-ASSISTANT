...
extern "C" {
    ...
    fn OpenAccessibilityPreferences();
    fn OpenInputMonitoringPreferences();
    fn OpenScreenRecordingPreferences();
...
pub fn is_can_input_monitoring(prompt: bool) -> bool {
    unsafe {
        let value = if prompt { YES } else { NO };
        let granted = InputMonitoringAuthStatus(value) == YES;
        if !granted && prompt {
...
fn unsafe_is_can_screen_recording(prompt: bool) -> bool {
    ...
    if !can_record_screen && prompt {
        unsafe { OpenScreenRecordingPreferences() };
    }
    can_record_screen
}
