on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set agent_plist to "/Library/LaunchAgents/fr.sfait.remoteassistant_server.plist"
  set daemon_plist to "/Library/LaunchDaemons/fr.sfait.remoteassistant_service.plist"
  set app_bundle to "/Applications/SFAIT Remote Assistant.app"
  ...
  set kill_others to "pids=$(pgrep -x 'SFAIT Remote Assistant' | grep -vx " & cur_pid & " || true); if [ -n \"$pids\" ]; then echo \"$pids\" | xargs kill -9 || true; fi;"
  ...
  do shell script sh with prompt "SFAIT Remote Assistant wants to update itself" with administrator privileges
end run
